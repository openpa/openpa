...
PEP 440 dev versions (e.g. ``0.1.9.dev12``) are translated to the SemVer
pre-release form (``0.1.9-dev.12``) on the way in. electron-builder and
npm validate strictly against SemVer and reject the PEP 440 dot-before-dev
form; stable releases pass through unchanged.
...
PEP440_DEV_RE = re.compile(r"^(\d+\.\d+\.\d+)\.dev(\d+)$")
