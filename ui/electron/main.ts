...
ipcMain.handle('openpa:backend-upgrade:apply', async (event, payload?: { targetVersion?: string }) => {
...
type InstallerRunPayload = {
  ...
  /** Optional explicit openpa package version, e.g. ``0.2.1`` (production)
...
async function runUninstaller(
  ...
  if (INSTALL_CHANNEL === 'dev') {
    ...
    send('openpa:installer:uninstall:log', { stream: 'info', line: `Using local install script: ${scriptPath} (--uninstall)` })
  } else {
    const scriptUrl = `${INSTALLER_SCRIPT_BASE}/${scriptName}`
    const scriptDir = path.join(app.getPath('userData'), 'installer')
    fs.mkdirSync(scriptDir, { recursive: true })
