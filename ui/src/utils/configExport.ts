...
export function buildMarkdownExport(s: ConfigExportSnapshot): string {
  ...
  lines.push('');
  lines.push(`- **Working directory:** \`${s.workingDir}\``);
  lines.push('');
  ...
  if (s.vnc) {
    ...
    lines.push('');
    lines.push(`- **Password:** \`${s.vnc.password}\``);
    lines.push('- **Web client:** http://localhost:6080');
    lines.push('- **VNC client:** localhost:5900');
    lines.push('');
  }
...
export function buildEnvExport(s: ConfigExportSnapshot): string {
  ...
  push(envLine('OPENPA_WORKING_DIR', s.workingDir));
  out.push('');
