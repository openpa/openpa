...
const form = ref({
  ...
  // OPENPA_WORKING_DIR — the user's interaction folder. Default for
...
<template>
  <div class="step-server-config">
    ...
    <ElForm label-position="top" class="config-form">
      ...
      <ElFormItem label="OpenPA System Working Directory">
        <ElInput
        ...
        />
        <div class="field-hint">
          OpenPA's internal storage — <code>bootstrap.toml</code>,
          ...
          <template v-if="!isFirstSetup">
            ...
            <code>OPENPA_SYSTEM_DIR</code> in
            <code>{{ form.system_dir }}/.env</code> and restart OpenPA.
          </template>
        </div>
      </ElFormItem>
