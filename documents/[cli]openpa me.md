...
acts on the active profile resolved from this token, `openpa me` is the
...
working directories), use `openpa me --json`.

## Global flags

`openpa me` accepts the root-level `--json` flag to emit the raw token
...
`openpa me` calls the server's identity endpoint with the bearer token and
...
```bash
$ openpa me --json | jq -r '.expires_at | todate'
...
`openpa setup complete` to mint one for the first profile.
...
**Wrong profile shown** — Profiles are baked into the token. If `openpa me`
reports a profile you did not expect, you are using the wrong token.
List available profiles with `openpa profile list` (under a token that has
