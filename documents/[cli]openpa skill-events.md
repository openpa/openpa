...
The page lists subscriptions in the left rail (matching `openpa skill-events list`),
shows recent notifications in the main area (matching
`openpa skill-events notifications`), and exposes a **Simulate** button on
each subscription row that opens a small editor for the markdown body
(matching `openpa skill-events simulate`).

## Streaming output format

`openpa skill-events stream` and `openpa skill-events notifications` both
...
All `openpa skill-events` subcommands accept the root-level `--json`
flag. `OPENPA_TOKEN` is required for every subcommand.

## Subcommands

### `openpa skill-events list`
...
```bash
$ openpa skill-events delete sub_19a8
```

### `openpa skill-events simulate`
...
```bash
openpa skill-events simulate <id> [--filename <name>]      # body read from stdin
```
...
```bash
# From a file
$ openpa skill-events simulate sub_19a8 --filename morning-brief.md < morning.md

# From a heredoc
$ openpa skill-events simulate sub_4f02 <<'EOF'
# PR opened
- Author: li
- Repo:  openpa
...
### `openpa skill-events notifications`
...
```bash
openpa skill-events notifications [--since <millis>]
```
...
```bash
$ openpa skill-events notifications --since 1746201600000
[notification] {"skill":"daily-brief","summary":"Daily brief ready","ts":1746205200000}
```

### `openpa skill-events events`
...
```bash
openpa skill-events events <skill>
```
...
```bash
$ openpa skill-events events daily-brief
{
...
### `openpa skill-events listener-status`
...
```bash
openpa skill-events listener-status <skill>
```
...
```bash
$ openpa skill-events listener-status daily-brief
skill_name      daily-brief
running         yes
...
### `openpa skill-events listener-start`
...
```bash
openpa skill-events listener-start <skill>
```
...
| Row             | Meaning                                                    |
|-----------------|------------------------------------------------------------|
| `process_id`    | Live process id (use with `openpa proc attach`).              |
| `autostart_id`  | Autostart registration id (use with `openpa proc autostart delete` to undo). |

**Example.**

```bash
$ openpa skill-events listener-start daily-brief
process_id    p_9d72
autostart_id  a_8c14
```
...
```bash
$ openpa skill-events notifications --json | jq '.summary'
"Daily brief ready"
"PR opened: openpa#42"
```
...
$ openpa skill-events notifications --since "$since"
```

### Manually fire a "morning" event for development

```bash
$ openpa skill-events simulate sub_19a8 <<'EOF'
# Morning brief
- Top issue: openpa#42
...
auth issue. Confirm `openpa me` works first.
...
`openpa skill-events listener-status <skill>` and start the listener if
needed.
...
process machinery. Use `openpa proc autostart list` to see whether a
duplicate or a stale entry is blocking the new one. If yes, delete the
stale row with `openpa proc autostart delete <id>` first.
