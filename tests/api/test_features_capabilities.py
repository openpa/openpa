...
name is in the list. A regression here means a newer Electron app
pinned to an older backend (the cross-version-install case
v0.1.9-test9's ``--version`` flag enables) would draw entries that
land on a 404 SPA fallback on click.
"""
...
def test_ui_features_list_matches_electron_tray_entries() -> None:
    """The Electron tray / jumplist / dock builders ([`ui/electron/main.ts`])
    call ``uiFeatureAvailable('processes' | 'events' | 'channels')`` —
    those names must be exactly the ones the backend ships. Drift on
    either side breaks the gate."""
...
def test_capabilities_response_includes_ui_features(
    ...
    gate. A missing field is the legacy contract — older backends ship
    without it and Electron falls back to "show all".
    """
    ...
    assert "ui_features" in body, (
        ...
        "would fall back to showing every tray entry."
    )
