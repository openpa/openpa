...
def _get_markitdown():
    ...
    Raises ``ImportError`` if the ``documents`` feature isn't installed so
    the caller can surface a structured ``MissingDependency`` result to the
    Reasoning Agent instead of silently disabling the tool.
    """
    global _markitdown
