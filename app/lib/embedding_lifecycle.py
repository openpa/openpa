...
def _drop_owned_collections(vector_store) -> None:
    ...
    for name in names:
        ...
        try:
            ...
            logger.exception(f"Failed to drop collection '{name}'")
...
def _rebuild_caches(*, agent, embedding, vector_store, profiles: list[str]) -> None:
    ...
    try:
        ...
        logger.exception("Failed to rebuild gg_places embedding table")
    ...
    for profile in profiles:
        try:
            ...
            logger.exception(f"Failed to rebuild tool embeddings for '{profile}'")
    ...
    try:
        ...
        logger.exception("Failed to reconcile documentation collection")
